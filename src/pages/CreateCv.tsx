...
export function CreateCv() {
  ...
  // Si on repasse en desktop, on reset l'onglet actif (affichage normal côte à côte)
  useEffect(() => {
    if (!isMobile) {
      setActiveTab('editor');
    }
  ...
  const ajouterExperience = () => setExperiences([...experiences, { poste: '', entreprise: '', description: '', dateDebut: '', dateFin: null }]);
  const modifierExperience = (index: number, champ: keyof Experience, valeur: string) => {
    const copie = [...experiences]; copie[index] = { ...copie[index], [champ]: valeur }; setExperiences(copie);
  };
  const supprimerExperience = (index: number) => setExperiences(experiences.filter((_, i) => i !== index));

  const ajouterFormation = () => setFormations([...formations, { diplome: '', etablissement: '', dateDebut: '', dateFin: null }]);
  const modifierFormation = (index: number, champ: keyof Formation, valeur: string) => {
    const copie = [...formations]; copie[index] = { ...copie[index], [champ]: valeur }; setFormations(copie);
  };
  ...
  const modifierCompetence = (index: number, champ: keyof Competence, valeur: string | number) => {
    const copie = [...competences]; copie[index] = { ...copie[index], [champ]: valeur }; setCompetences(copie);
  };
  ...
  const modifierLangue = (index: number, champ: 'nom' | 'niveau', valeur: string | number) => {
    const copie = [...langues]; copie[index] = { ...copie[index], [champ]: valeur }; setLangues(copie);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErreur(''); setUploadError('');
    if (envoiEncours) return;
    setenvoiEncours(true);

    try {
      const createdCv = await createCv({
        nom, prenom, dateNaissance, adresse, email, telephone, experiences, formations, competences, situationMatrimoniale, loisirs, nationalite, permis, resume, langues,
      });
  ...
  const cvPreviewData: CvDto = {
    id: 0, photoUrl: photoPreview || undefined, nom, prenom, dateNaissance, adresse, email, telephone,
    resume, situationMatrimoniale, nationalite, permis, loisirs, experiences, formations, competences, langues
  };
      ...
      <div className={`editor-sidebar ${!isMobile || activeTab === 'editor' ? 'active' : ''}`}>
        <div className="editor-header">
          ...
          <button onClick={handleSubmit} className="landing-btn landing-btn--primary" disabled={envoiEncours}>
            <FiSave /> {envoiEncours ? 'Enregistrement...' : 'Sauvegarder'}
          </button>
        </div>
